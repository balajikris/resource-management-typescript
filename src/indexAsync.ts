...
class ResourceManagementSample {
  ...
  public async execute(): Promise<boolean> {
    try {
      let credentials = await this.login();
      this.resourceClient = new ResourceManagementClient(credentials, this.state.subscriptionId)

      let resourceGroup = await this.createResourceGroup();
      console.log(`\n-->result of create resource group operation is ${JSON.stringify(resourceGroup)}`);

      let resourceGroups = await this.listResourceGroups();
      console.log(`\n-->result of list resource groups operation is ${JSON.stringify(resourceGroups)}`)

      let updatedRg = await this.updateResourceGroup();
      console.log(`\n-->result of update resource group operation is ${JSON.stringify(updatedRg)}`)

      let rgWithResource = await this.createResource();
      console.log(`\n-->result of create resource operation is ${JSON.stringify(rgWithResource)}`)

      let res = await this.getResource();
      console.log(`\n-->result of get resource operation is ${JSON.stringify(res)}`)

      let rgExportRes = await this.exportResourceGroupTemplate();
  ...
  private login(): Promise<msRestAzure.ApplicationTokenCredentials> {
    return msRestAzure.loginWithServicePrincipalSecret(this.state.clientId, this.state.secret, this.state.domain, this.state.options);
  }
